...
load_data_from_TiSA<-function(time_object,gtf_data,chrom_order=c(paste0("chr", 1:22), "chrX", "chrY"),l2fc_colors=c('blue','red'),pval_type='padj'){
  dta_lst<-list()
  for(exp in names(time_object@DE_results$conditional)){
    temp_df<-time_object@DE_results$conditional[[exp]]$DE_sig_data
    temp_df<-temp_df[temp_df$gene_id %in% target_genes,c('gene_id','log2FoldChange',pval_type)]
    #Get gtf_data
...
compute_size_same_scale<-function(data_list,target_column='padj'){
  ...
  global_min <- min(sapply(data_list, function(df) min(-log10(df[[target_column]]), na.rm = TRUE)))
  global_max <- max(sapply(data_list, function(df) max(-log10(df[[target_column]]), na.rm = TRUE)))

  # Scale the target_column column for each dataframe
  data_list <- lapply(data_list, function(df) {
    df$size <- 0.3 + (-log10(df[[target_column]]) - global_min) * (1 - 0.3) / (global_max - global_min)
    return(df)
  })
  return(data_list)
}
...
create_TiSA_circos<-function(data_list,cytoband_data,genomic_lines_list, highlight_chrom=NULL, clust_label_size=0.1,
  ...
  circos.initializeWithIdeogram(cytoband_data)
  ...
  sector_limits <- get.cell.meta.data("xlim", sector.index = "clusters", track.index = heatmap_track)
  track_ylims <- get.cell.meta.data("ylim", sector.index = "clusters", track.index = heatmap_track)
  ...
  for (i in seq_along(positions)) {
    circos.text(x = positions[i], y = track_ylims[2] + 0.5, # Place labels slightly outside the track
                ...
                sector.index = "clusters", track.index = heatmap_track, cex=clust_label_size, adj = c(0.5, 5.5) # Center the labels
    )
  }
...
#Load the data
load('/media/yohanl/MAPLE_STICK/timeseries_obj_res.Rdata')
...
my_gtf$gene_id<-unlist(strsplit(my_gtf$gene_id,'\\.'))[c(T,F)]
colnames(my_gtf)[colnames(my_gtf)=='gene_name']<-'Nope'
colnames(my_gtf)[colnames(my_gtf)=='gene_id']<-'gene_name'

#Check genes which are not available?
unique(target_genes[!target_genes %in% my_gtf$gene_id])
...
#Remove strange seqids
my_gtf<-my_gtf[my_gtf$seqid %in% c(paste0("chr", 1:22), "chrX", "chrY"),]
my_gtf$seqid<-factor(my_gtf$seqid,levels=c(paste0("chr", 1:22), "chrX", "chrY"))
...
dta_lst<-load_data_from_TiSA(time_object = TS_object,gtf_data = my_gtf,pval_type='pvalue')

#Compute the size on a shared scale
dta_lst<-compute_size_same_scale(dta_lst,target_column = 'pvalue')
...
genomic_lines_and_gtf_lst$gtf_mod[genomic_lines_and_gtf_lst$gtf_mod$gene_id=='ENSG00000089195',]

test<-dta_lst$KD_T6_61a_vs_SiC_TP_0
test[test$start==5937228,]


test[test$"seqid"=='chr20',]


my_gtf[my_gtf$seqid=='chr20',]

-log10(test$pvalue[test$"seqid"=='chr20'])

# Get the global min and max of the 'padj' column across all dataframes
global_min <- min(sapply(dta_lst, function(df) min(df[['pvalue']], na.rm = TRUE)))
global_max <- max(sapply(dta_lst, function(df) max(df[['pvalue']], na.rm = TRUE)))
