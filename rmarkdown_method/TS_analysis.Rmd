---
title: "TimeSeries Analysis – TS example"
author: "Yohan Lefol"
...
# Rscript -e "rmarkdown::render('TS_analysis.Rmd',output_file='TS_analysis.html')"
library(knitr)
library(TimeSeriesAnalysis)
packages_for_loading<-c('DESeq2','limma','clusterGenomics','gprofiler2','tictoc',
                        'tibble','GOSemSim','dplyr','tibble','grid','ggrepel',
                        'ggplot2','ComplexHeatmap','reshape2','plotly',
                        'GO.db','data.table','htmlwidgets','dynamicTreeCut','stringr')
...
name_result_folder<-'TS_results_example/'
obj_name<-'timeSeries_obj_example.Rdata'

#Path to count data and sample data respectively
my_path_data<-'data/raw_counts_TS'
my_path_sample_dta<-'data/sample_file.csv'
...
```{r organism_installation, echo=FALSE, message=FALSE, warning=FALSE}
#Installs the organism declared 
...
sample_dta<-TS_object@sample_data
row.names(sample_dta)=NULL

```
## TimeSeries summary of experiment
**Name of groups beings compared\:** `r paste0(TS_object@group_names[1],' vs ',TS_object@group_names[2])`  
**Number of genes analyzed\:** `r length(row.names(TS_object@count_matrix$norm))`  

**Differential expression parameters used\:**  
**Method\:** `r TS_object@DE_method`  
**P statistic filter used\:** `r TS_object@DE_p_filter` with a `r TS_object@DE_p_thresh` threshold  
**Log(2)FoldChange significance threshold used\:** `r TS_object@DE_l2fc_thresh`  

**PART parameters used\:**  
**PART log(2)FoldChange significance threshold\:** `r TS_object@PART_l2fc_thresh`  
**number of recursions\:** `r TS_object@PART_results$params_used$part_recursion`  
**minimum cluster size\:** `r TS_object@PART_results$params_used$part_min_clust`  
**distance parameter\:** `r TS_object@PART_results$params_used$dist_param`  
**hclust parameter\:** `r TS_object@PART_results$params_used$hclust_param`  
**custom seed used\:** `r as.character(TS_object@PART_results$params_used$custom_seed)`  
**PART computation time\:** `r TS_object@PART_results$comp_time`  
...
gene_matrix_interest<-TS_object@count_matrix$norm[row.names(TS_object@count_matrix$norm) %in% genes_of_interest,]
if(nrow(gene_matrix_interest)>0){
    var_vect <- apply(gene_matrix_interest, 1, stats::var)
    selected_gene<-names(sort(var_vect,decreasing=T))[1]
    target_file<-paste0(name_result_folder,'genes_of_interest/',selected_gene,'_trajectory.png')
    knitr::include_graphics(target_file)
}else{
  selected_gene<-row.names(TS_object@PART_results$cluster_map)[1]
  gene_traj_dta<-calculate_gene_traj_data(TS_object,selected_gene,log_tp_traj)
  gene_traj_plot<-plot_single_gene_traj(gene_traj_dta,TS_object@group_colors)
  gene_traj_plot
}
...
plt_clust<-plot_cluster_traj(object = TS_object,ts_data = sub_ts,ts_mean_data = sub_mean, num_col=1,rem_legend_axis=T)
...
The species parameter are used to calculate the semantic distance which impacts
...
CC – Cellular Components, and BP – Biological processes).  
...
MDS_plots<-wrapper_MDS_and_MDS_clusters(GO_clusters,sem_data,my_ont_sem_sim,target_dir=paste0(name_result_folder,'gprofiler_results/'),return_plot=T)
```
...
if('REAC_dotplot.svg' %in% list.files(loc_gpro)==T){
  knitr::include_graphics(paste0(loc_gpro,"REAC_dotplot.svg"))
}else if('GO:BP_dotplot.svg' %in% list.files(loc_gpro)==T){
  knitr::include_graphics(paste0(loc_gpro,"GO:BP_dotplot.svg"))
}else if('KEGG_dotplot.svg' %in% list.files(loc_gpro)==T){
...
if(is.null(ancestor_plots[['MDS']])==F){
  ancestor_plots[['MDS']]
}
```
