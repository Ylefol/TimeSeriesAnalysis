...
if (TS_object@DE_method=='DESeq2' & 'norm' %in% names(TS_object@count_matrix)!=TRUE){
...
MDS_plots<-wrapper_MDS_and_MDS_clusters(GO_clusters,TS_object@sem_list,my_ont_sem_sim,target_dir=paste0(name_result_folder,'gprofiler_results/'),return_plot=TRUE)
```
...
if(length(target_ancestors)==0){
  ...
  ancestor_plots<-wrapper_ancestor_curation_plots(GOs_ancestors_clust,TS_object@sem_list,return_plot=TRUE,target_dir=name_result_folder)
}
